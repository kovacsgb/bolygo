...
C Global variable(s):
C      INTEGER IT, NT      
C      REAL*8 eD, eG, rho, T, T0, T1, dT, aKext
C      DIMENSION eD(5,6) 
C      DIMENSION eG(71,71)
C      CHARACTER*3 model  
C      CHARACTER*1 top    
C      CHARACTER*1 shape  
C      LOGICAL ross
...
C      if (ross) then
C         open(unit=99,file='kR.out',status='unknown',
C     &        access='append')
C         print  *, 'Calculation of Rosseland mean opacities '        
C         write(99,*), 'Calculation of Rosseland mean opacities '        
C      else
C         open(unit=99,file='kP.out',status='unknown',
C     &        access='append')      
C         print  *, 'Calculation of Planck mean opacities '        
C         write(99,*), 'Calculation of Planck mean opacities '        
C      end if
C      rewind 99
C Initialization of all necessary data for a chosen dust model:
C      call init_d(ross,model,top,shape,eD)
C Initialization of data for the gas model:
C      call init_g(ross,eG)
C  
C  Start loop by temperatures:
C
C          T = T0
C         dT = DEXP(DLOG(T1/T0)/(NT-1))
C      DO IT = 1, NT
C-----------------------------------------------------------------------------
...
C-----------------------------------------------------------------------------
C         CALL cop(eD,eG,rho,T,aKext)
C Write results to the output: 
C         WRITE(99,'(2X,F10.4,1X,E15.6)') T, aKext
...
C Exit:        
C END program
       subroutine calculate_table(model,top,shape,ross,T0,T1,NT,rho0,
     &  rho1,Nrho,opac)
       implicit none
       INTEGER IT, NT,Nrho, IR
       REAL*8 eD, eG, rho,rho0,rho1, T, T0, T1, dT, aKext, drho
       real*8 opac
       dimension opac(*)
       DIMENSION eD(5,6) 
       DIMENSION eG(71,71)
       ...
       drho=DEXP(DLOG(rho1/rho0)/(Nrho-1))

       do IT = 1, NT, 1
             rho=rho0
             do IR = 1, Nrho,1
                  call cop(ed,eG,rho,T,aKext)
                  opac((IT-1)*Nrho+IR) = aKext
                  rho=rho*drho
             end do
             T=T*dT
       end do

...
!     ..................................................................
!     .  License: one can freely use, modify, or redistribute all parts 
!     .  code.                                                          
!     .  [Please, inform me about all bugs which you will find in the co
!     ..................................................................
!     .  DESCRIPTION OF THE MODEL:                                      
!     ..................................................................
!     .  Calculation of the Rosseland and Planck mean opacities of the g
!     .  and dust in the temperature range 5[K]<T<~10,000[K] and for gas
!     .  density between ~2*10^-18 [g/cm^3] and ~2*10^-7[g/cm^3]. The so
!     .  composition of the elements is adopted from the compilations of
!     .  Anders & Grevesse (1989, all but carbon) and Grevesse et al.   
!     .  (1991, carbon).                                                
!     .                                                                 
!     .  It is supposed that in the temperature range 0<T<~1500[K] (depe
!     .  on the gas density) the opacity is dominated by dust grains, wh
!     .  for higher temperatures gas species are the only possible sourc
!     .  the opacity.                                                   
!     ..................................................................
!     .  I. DUST OPACITIES:                                             
!     ..................................................................
!     .  Dust grains are supposed to consist of silicates, iron, troilit
!     .  organics, and ice (Pollack et al. 1994, Henning & Stognienko 19
!     .  We applied a modified MRN size distribution as proposed by Poll
!     .  al. (1985):                                                    
!     .                                                                 
!     .  a~a^-q1, amin<a<amax1,                                         
...
!     .                                                                 
!     .  The silicates - amorphous olivine and pyroxene - are considered
!     .  three various types, depending on their iron content. However, 
!     .  absolute amount of solid metallic iron in the model is kept unc
!     .                                                                 
!     .  1) "iron-poor" silicates, Fe/(Fe+Mg)=0.0 ("high" Fe abundance),
!     .  2) "iron-rich" silicates, Fe/(Fe+Mg)=0.4 ("low" Fe & FeS abunda
!     .  3) "normal" silicates,    Fe/(Fe+Mg)=0.3 ("normal" Fe abundance
!     .                                                                 
!     .  We modelled dust grains as aggregates or spherical particles wi
!     .  different distribution of the dust constituents:               
!     .                                                                 
!     .  a) Homogeneous dust particles, where each particle consists of 
...
!     .                                                                 
!     .  c) Porous composite particles, where each particle consists of 
...
!     .  II. GAS OPACITIES:                                             
!     ..................................................................
!     .  For temperatures between roughly 1500 [K] and 10,000 [K], where
!     .  dust grains have evaporated, a new gas opacity table of the Ber
!     .  group (Ch. Helling and E. Sedlmayr) is used (Ch. Helling et al.
!     .  Ch. Helling 2001, 2002, private communications). We wrote a spe
!     .  subroutine "eint" to interpolate these data for a chosen temper
!     .  and density.                                                   
!     .                                                                 
!     ..................................................................
!     .  III. DUST-TO-GAS TRANSITIONAL REGION (T~1500 K):               
!     ..................................................................
!     .  For this region, our model may produce unreliable results as th
...
!     .  authors, this approach is still quite accurate (eg. Lenzuni et 
!     .  1995). The reason is that the gas opacities are much, much lowe
!     .  (about few orders of magnitude) then the dust opacities. Given 
!     .  evaporation of the last refractory dust materials occurs under 
!     .  restricted temperature range (~200 K), one can simply make the 
!     .  interpolation between purely gas-dominated and dust-dominated  
!     .  opacities for such temperatures without introducing significant
!     .                                                                 
!     ..................................................................
!     .  IV.REFERENCES:                                                 
!     ..................................................................
!     .  1) Anders, E., Grevesse, N. (1989), Geochim. Cosmochim. Acta, 5
!     .  2) Grevesse, N., Lambert, D.L., Sauval et al. (1991), A&A, 242,
!     .  3) Helling, Ch., Winters, J., Sedlmayr, E. (2000), A&A, 358, 65
!     .  4) Henning, Th., Stognienko, R. (1996), A&A, 311, 291          
!     .  5) Lenzuni, P., Gail, H.-P., Henning, Th. (1995), ApJ, 447, 848
!     .  6) Pollack, J., McKay, C.P., Christofferson, B.M. (1985), Icaru
!     .     471                                                         
...
!     .  Rosseland and Planck mean dust opacities can be accurately repr
!     .  in the form of a polynom of the fifth degree in every temperatu
!     .  region. We considered 5 different temperature regions:         
!     .    1. All dust components are present: 5 K<T<~100 K,            
!     .    2. All dust components but water ice are present: ~100 K<T<27
!     .    3. Silicates, iron, troilite, and refractory organics are pre
!     .       275 K<T<425 K,                                            
!     .    4. Silicates, iron and troilite are present: 425 K<T<680 K,  
!     .    5. Silicates and iron are present: 680 K<T<~1500 K.C         
!     .  In our model, we added a weak dependence of evaporation tempera
!     .  of ice, silicates, and iron on the gas density.                
!     .                                                                 
!     .  The code uses these pre-computed polynomial coefficients for al
!     .  possible dust models. Unfortunately, it is not possible to appl
...
!     .  2) "y" means the distribution of dust materials within the part
...
!     .  "kR_h2001.dat" (Rosseland) and "kP_h2001.dat" (Planck). The cod
!     .  makes two-order interpolation within these data, when it is nec
!     .  to compute gas opacities, with routine "eint".                 
!     .                                                                 
!     .  The main subroutine is "COP" ("Calculation of OPacities"). For 
!     .  chosen dust model, the opacity kind and given gas temperature a
...
!     .            = 'nrm' - "normal" silicate dust model,    Fe/(Fe+Mg)
!     .            = 'ips' - "iron-poor" silicate dust model, Fe/(Fe+Mg)
!     .            = 'irs' - "iron-rich" silicate dust model, Fe/(Fe+Mg)
...
!     .        'aKext': a set of corresponding mean opacities (extinctio
!     .                                                                 
!     ..................................................................
!     .  SUBROUTINES (alphabetically):                                  
!     ..................................................................
!     .    'bint' - quadratic interpolation routine to compute the evapo
...
!      DO IT = 1, NT                                                    
!-----------------------------------------------------------------------
! Calculation of Rosseland or Planck mean opacities using a chosen densi
! temperature, silicate dust model, shape of the grains and their topolo
!-----------------------------------------------------------------------
...
!.......................................................................
! Subroutine that initialize the polynomial fit coefficients for a chose
...
! Local variable(s):                                                    
      ...
      CHARACTER*80 kR_H, kP_H 
                                      !a file with Rosseland mean gas op
      PARAMETER (kR_H='kR_h2001.dat', 
                                      !a file with Planck mean gas opaci
                 kP_H='kP_h2001.dat') 
      DIMENSION seq(71*71) 
!                                                                       
! Open input file:                                                      
...
! Initialization of mean gas opacities:                                 
      ...
      read (10,'(d10.8)') xmy 
                                              !reading data set,        
      read (10,'(7f12.4)') (seq(k),k=1,71*71) 
...
! Local variable(s):                                                    
      ...
      LOGICAL SMOOTH 
! Data related to the evaporation temperatures of silicates, iron and ic
      ...
      DATA tt(1,1),tt(2,1),tt(3,1),tt(4,1), & 
           ...
           tt(5,4),tt(6,4),tt(7,4),tt(8,4) & 
                                                     !Water ice evaporat
          /1.090E+02,1.180E+02,1.290E+02,1.430E+02, 
                                                     !temperature, &    
           1.590E+02,1.800E+02,2.070E+02,2.440E+02, 
                                                     !then metallic iron
           8.350E+02,9.080E+02,9.940E+02,1.100E+03, 
                                                     ! &                
           1.230E+03,1.395E+03,1.612E+03,1.908E+03, 
                                                     !orthopyroxene &   
           9.020E+02,9.800E+02,1.049E+03,1.129E+03, 
                                                     !and &             
           1.222E+03,1.331E+03,1.462E+03,1.621E+03, 
                                                     !olivine &         
           9.290E+02,9.970E+02,1.076E+03,1.168E+03, 
                                                     !depending on densi
           1.277E+03,1.408E+03,1.570E+03,1.774E+03/ 
!                                                    !'ro'              
! Initialization of the output parameters:                              
      aKext = 0.0D0 
                                !T must be more that few degrees [K]    
      if (T_in.lt.1.0D0) RETURN 
!.......................................................................
! Constant(s):                                                          
!.......................................................................
                                !Pi                                     
...
!.......................................................................
! Interpolation of a matrix of evaporation temperatures for a given dens
! 'rho_in':                                                             
!.......................................................................
      DO I = 1,4 
         DO J = 1,8 
...
!                                                                       
!  'D': D(1:N,1:M) - two-dimensional table to be interpolated/extrapolat
!                                                                       
...
!.......................................................................
!  Fit coefficients for "normal" silicate dust model, Fe/(Fe+Mg)=0.3 for
!  Rosseland and Planck mean opacities (extinction) in the case of compo
!  aggregates.                                                          
!.......................................................................
!  Input parameter(s):                                                  
!.......................................................................
...
! Fit coefficients for Planck mean opacity (extinction)                 
      ...
      END                                           
!.......................................................................
!  Fit coefficients for "iron-rich" silicate dust model, Fe/(Fe+Mg)=0.4 
!  Rosseland and Planck mean opacities (extinction) in the case of      
!  homogeneous spherical particles.                                     
!.......................................................................
!  Input parameter(s):                                                  
...
! Fit coefficients for Planck mean opacity (extinction)                 
      ...
      END                                           
!.......................................................................
!  Fit coefficients for "iron-poor" silicate dust model, Fe/(Fe+Mg)=0 fo
!  Rosseland and Planck mean opacities (extinction) in the case of      
!  homogeneous spherical particles.                                     
!.......................................................................
!  Input parameter(s):                                                  
...
!.......................................................................
!  Fit coefficients for "iron-poor" silicate dust model, Fe/(Fe+Mg)=0 fo
!  Rosseland and Planck mean opacities (extinction) in the case of porou
!  composite spherical particles.                                       
!.......................................................................
!  Input parameter(s):                                                  
!.......................................................................
...
!.......................................................................
!  Fit coefficients for "normal" silicate dust model, Fe/(Fe+Mg)=0.3 for
!  Rosseland and Planck mean opacities (extinction) in the case of compa
!  5-layered spherical particles.                                       
!.......................................................................
!  Input parameter(s):                                                  
!.......................................................................
